...
from ISDA_try import ISDA_expansion
...
def format_batch(batch):
    ...
    # 堆疊資料與標籤
    stacked_data = tf.stack([data_c1, data_c2], axis=0)  # (2, batch_size, data_dim)
    stacked_label = tf.stack([label_c1, label_c2], axis=0)  # (2, batch_size)
...
class Jammer(tf.keras.Model):
    ...
    def call(self, generated, real):
        query = self.query_layer(real)
        key = self.key_layer(generated)
        value = self.value_layer(generated)
        # query = real
        # key = generated
        # value = generated
...
class Jam_GAN(tf.keras.Model):
    def __init__(self, input_dim):
        ...
        self.function_f = ISDA_expansion.function_f(input_dim=Parameter.hidden_dim, embedding_dim=units,
                        name=Parameter.name)
        self.classifier = ISDA_expansion.classifier_g(embedding_dim=units, num_classes=Parameter.num_classes,
                        name=Parameter.name)
        ...
        self.discriminator_optimizer = optimizers.Adam(learning_rate=0.001, beta_1=0.5)
        self.discriminator_rf_optimizer = optimizers.Adam(learning_rate=0.001, beta_1=0.5)
        self.jammer_optimizer = optimizers.Adam(learning_rate=0.0005, beta_1=0.5)
        self.acc = tf.keras.metrics.SparseCategoricalAccuracy()
        self.criterion = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)
        self.optimizer = optimizers.Adam(learning_rate=initial_learning_rate, beta_1=0.5)
    ...
    def fit(self, train_data, train_label, test_data, test_label, batch_size, num_epochs, cls_epochs, mode, training=True):
        ...
        if training:
            ...
            for step in range(ratio):
                ...
                for epoch in range(gan_epochs):
                    ...
                    if mode == 'gd':
                        ...
                        self.d_ratio, self.g_ratio, self.j_ratio = 3, 5, 5
                ...
                #     './ours_weight/classifier_g/ISDA_g_model_weights_{}.h5'.format(Parameter.name))
                ...
                for epoch in range(cls_epoch):
                    ...
                    if (epoch + 1) % 1 == 0:
                        ...
                        if acc > best_acc:
                            ...
                            # 儲存暫時最佳分類器權重
                            self.classifier.save_weights('./ours_weight/classifier_g/ISDA_temp_best_weights.h5')

                if best_acc > total_best_acc:
                    total_best_acc, total_best_auroc, total_best_f1 = best_acc, best_auroc, best_f1
                    self.classifier.load_weights('./ours_weight/classifier_g/ISDA_temp_best_weights.h5')
                    self.classifier.save_weights(
                        './ours_weight/classifier_g/ISDA_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
            ...
            self.classifier.load_weights('./ours_weight/classifier_g/ISDA_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
            val_data = tf.reshape(test_data, [-1, 200])
            val_labels = tf.reshape(test_label, [-1])
            # 將資料和標籤轉成 numpy array
            val_data_np = np.array(val_data)
            val_labels_np = np.array(val_labels)

            # 產生隨機索引並 shuffle
            indices = np.arange(len(val_data_np))
            np.random.shuffle(indices)

            # 用隨機索引做 shuffle
            val_data_shuffled = val_data_np[indices]
            val_labels_shuffled = val_labels_np[indices]
    ...
    def train_step(self, inputs, mode=None, num=None):
        """
        Do not decorate train_step with `tf.function`, since 3 optimizers in it.
        ...
        with tf.GradientTape(persistent=True) as gen_tape, tf.GradientTape(persistent=True) as disc_tape,tf.GradientTape(persistent=True) as disc_rf_tape, tf.GradientTape(persistent=True) as jam_tape:
            ...
            for cls in range(Parameter.num_classes):
                ...
                # G_class_loss = tf.constant(0.0, dtype=tf.float32)
                G_class_loss = G_gen_loss_class[cls] if cls < len(G_gen_loss_class) else 0.0
                # G_rf_loss = tf.constant(0.0, dtype=tf.float32)
    ...
    def evaluate(self, val_data, val_labels):
        ...
        for j in range(len(predictions_new)):  # len(predictions_new[i])=16
            if np.all(predictions_new[j] == val_labels[j]):
                correct_count += 1
            total_count += 1
    ...
    def take_generated(self, generated, training=False):
        ...
        # print("output_real_fake_logits:", output_real_fake_logits)
    ...
    def compute_accuracy(self, org_data, generated_data):
        ...
        for c in range(Parameter.num_classes):
            ...
            #     tf.cast(tf.argmax(fake_pred, axis=1) == fake_g_labels, tf.float32))  # 被 D 誤判為真實數據的比例, label全為1
            G_accuracy = tf.reduce_mean(
    ...
    def save_all_weights(self, save_dir):
        ...
        for i, generator in enumerate(self.Generator_list):
            ...
            file_path = f"{dir_path}/ISDA_{Parameter.name}_G_weights"
        ...
        for i, discriminator in enumerate(self.Discriminator_list):
            ...
            file_path = f"{dir_path}/ISDA_{Parameter.name}_D_weights"
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)  # 创建目录
            if os.path.exists(file_path):
                os.remove(file_path)  # 删除已经存在的文件，确保不会重复创建组
            ...
            file_path = f"{dir_path}/ISDA_{Parameter.name}_Drf_weights"
            # 確保目錄存在
            if not os.path.exists(dir_path):
        ...
        file_path = f"{dir_path}/ISDA_{Parameter.name}_J_weights"
        # 確保目錄存在
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        # 如果檔案已經存在，先刪除確保不重複
    ...
    def load_all_weights(self, save_dir):
        for i, generator in enumerate(self.Generator_list):
            ...
            file_path = f"{save_dir}/generator/generator_{i}/ISDA_{Parameter.name}_G_weights"
            print(i)
        ...
        for i, discriminator in enumerate(self.Discriminator_list):
            ...
            file_path = f"{save_dir}/discriminator/discriminator_{i}/ISDA_{Parameter.name}_D_weights"
        ...
        # 設定要載入的權重路徑
        file_path = f"{save_dir}/Discriminator_rf/Drf/ISDA_{Parameter.name}_Drf_weights"
        ...
        # 設定要載入的權重路徑
        file_path = f"{save_dir}/jammer/jammer/ISDA_{Parameter.name}_J_weights"
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        plt.savefig(f'ISDA_ours_{Parameter.name}_TSNE.png', dpi=300, bbox_inches='tight')
        np.savez(
            f'ISDA_ours_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
        ...
        with open(f'ISDA_ours_{Parameter.name}_TSNE_data.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    for run in range(num_runs):
        ...
        history = gan_model.fit(phi_data_tensor, label_tensor, test_phi_data_tensor, val_label_tensor,
                                      batch_size=batch_size, num_epochs=gan_epochs, cls_epochs=cls_epoch,
                                        mode='gd', training=True)
    ...
    # 將結果寫入文字檔
    with open(f"ISDA_ours_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 ISDA_ours_{Parameter.name}_results.txt")
