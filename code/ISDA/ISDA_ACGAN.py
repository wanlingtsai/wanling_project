...
class acgan(tf.keras.Model):
    def __init__(self, latent_dim, label_dim, name):
        ...
        self.function_f = ISDA_expansion.function_f(input_dim=Parameter.hidden_dim, embedding_dim=latent_dim, name=Parameter.name)
        ...
        self.opt_g = tf.keras.optimizers.Adam(learning_rate=initial_learning_rate, beta_1=0.5)
        self.opt_d = tf.keras.optimizers.Adam(learning_rate=initial_learning_rate, beta_1=0.5)
    # @tf.function
    ...
    class get_discriminator(tf.keras.Model):
        def __init__(self, latent_dim, name):
            ...
            self.classifier_g = ISDA_expansion.classifier_g(latent_dim, num_classes=Parameter.num_classes, name=name, units=units)
            self.d_name = name
            self.latent_dim = latent_dim    #1920
        ...
        def build(self, input_shape):
            ...
            self.LayerD_dense = [
                layers.Dense(units=1,
                             ...
                             kernel_initializer=create_initializer(),
                             bias_initializer=create_initializer(),
                             name='d{}_l1_sigmoid'.format(self.d_name)),
    ...
    def ganfit(self, data_d, labels, val_data, val_labels, batch_size, num_epochs, training):
        ...
        if training:
            ...
            for ep in range(num_epochs):
                ...
                val_F1 = []
                # self.d.load_weights('./ACGAN_weight/ISDA_d_weight/ISDA_val_d_weights_{}.h5'.format(Parameter.name))
                for val_batch_data, val_batch_labels in val_ds:
                    ...
                    # 計算 F1-score（二分類）
                    f1 = f1_score(val_labels_array, predictions_new, average='binary')
                    # one batch accuracy
                    accuracy = correct_count / total_count
                    # record one epoch acc, auroc, after the end of epoch should be initialized
                ...
                if val_ep_acc > val_best_acc:
                    ...
                    self.d.save_weights('./ACGAN_weight/ISDA_d_weight/ISDA_val_d_weights_{}.h5'.format(Parameter.name))
                    self.g.save_weights('./ACGAN_weight/ISDA_g_weight/ISDA_val_g_weights_{}.h5'.format(Parameter.name))
    ...
    def evaluate(self, val_data, val_label):
        ...
        with open(f"ISDA_ACGAN_test_metric_{Parameter.name}.json", "w") as f:
    ...
    def save_all_weights(self, path):
        """儲存生成器與判別器的權重"""
        # 儲存 Generator 權重
        self.g.save_weights(f"{path}/ISDA_generator_weights_{Parameter.name}.h5")
        # 儲存 Discriminator 權重
        self.d.save_weights(f"{path}/ISDA_discriminator_weights_{Parameter.name}.h5")
        print("生成器與判別器的權重已儲存。")
    ...
    def load_all_weights(self, path):
        ...
        # 載入權重
        self.g.load_weights(f"{path}/ISDA_generator_weights_{Parameter.name}.h5")
        self.d.load_weights(f"{path}/ISDA_discriminator_weights_{Parameter.name}.h5")
        print("生成器與判別器的權重已載入。")
...
if __name__ == '__main__':
    ...
    for run in range(num_runs):
        ...
        d_model.load_weights('./ISDA_weight/classifier_g/g_model_weights_{}.h5'.format(Parameter.name))
        print("Trainable variables for classifier_g:")
        for var in gan_model.d.classifier_g.trainable_variables:
            print(f"{var.name}: {var.numpy()}, dtype={var.dtype}")
        ...
        result = gan_model.ganfit(phi_data_tensor, label_tensor, test_phi_data_tensor, val_label_tensor,
    ...
    print("std F1:", f1_std)
    # 將結果寫入文字檔
    with open(f"ISDA_ACGAN_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 ISDA_ACGAN_{Parameter.name}_results.txt")
    print("-------------------------------------------------------------")
