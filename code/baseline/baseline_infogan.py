...
class TotalModel(tf.keras.Model):
    ...
    def fit(self, train_data, test_data, test_label, batch_size, num_epochs, cls_epochs, training):
        ...
        if training:
            ...
            for i in range(int(ratio)):
                ...
                for epoch in range(cls_epochs):
                    ...
                    if test_epoch_accuracy > best_acc:
                        ...
                        self.classifier.save_weights(
                            f'./infoGAN_weight/classifier_g/baseline_temp_best_weights_{Parameter.name}.h5')
                ...
                if best_acc > total_best_acc:
                    ...
                    self.classifier.load_weights(
                        f'./infoGAN_weight/classifier_g/baseline_temp_best_weights_{Parameter.name}.h5')
                    self.classifier.save_weights(
                        './infoGAN_weight/classifier_g/baseline_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
            ...
            for test_batch_data, test_batch_label in combined_test_dataset:
                ...
                correct_predictions += np.sum(preds == test_batch_label)
    ...
    def save_all_weights(self, path):
        ...
        self.infogan1.generator.save_weights(f"{path}/Baseline_infogan1_generator_weights_{Parameter.name}.h5")
        self.infogan1.discriminator.save_weights(f"{path}/Baseline_infogan1_discriminator_weights_{Parameter.name}.h5")
        self.infogan1.q_network.save_weights(f"{path}/Baseline_infogan1_q_network_weights_{Parameter.name}.h5")

        self.infogan2.generator.save_weights(f"{path}/Baseline_infogan2_generator_weights_{Parameter.name}.h5")
        self.infogan2.discriminator.save_weights(f"{path}/Baseline_infogan2_discriminator_weights_{Parameter.name}.h5")
        self.infogan2.q_network.save_weights(f"{path}/Baseline_infogan2_q_network_weights_{Parameter.name}.h5")
        print("所有模型的權重已儲存。")
    ...
    def load_all_weights(self, path):
        ...
        self.infogan1.generator.load_weights(f"{path}/Baseline_infogan1_generator_weights_{Parameter.name}.h5")
        self.infogan1.discriminator.load_weights(f"{path}/Baseline_infogan1_discriminator_weights_{Parameter.name}.h5")
        self.infogan1.q_network.load_weights(f"{path}/Baseline_infogan1_q_network_weights_{Parameter.name}.h5")
        ...
        self.infogan2.generator.load_weights(f"{path}/Baseline_infogan2_generator_weights_{Parameter.name}.h5")
        self.infogan2.discriminator.load_weights(f"{path}/Baseline_infogan2_discriminator_weights_{Parameter.name}.h5")
        self.infogan2.q_network.load_weights(f"{path}/Baseline_infogan2_q_network_weights_{Parameter.name}.h5")
        print("所有模型的權重已載入。")
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        # 儲存圖像
        save_path = f"baseline_infoGAN_{Parameter.name}_TSNE.png"  # 你可以改成其他格式，例如 "ours_moex_fig.pdf"
        fig.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"圖像已儲存至 {save_path}")
        np.savez(
            f'baseline_infoGAN_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
        ...
        with open(f'baseline_infoGAN_{Parameter.name}_TSNE_data.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    # 將結果寫入文字檔
    with open(f"Baseline_infoGAN_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 Baseline_infoGAN_{Parameter.name}_results.txt")
