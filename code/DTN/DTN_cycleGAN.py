...
class TotalModel(tf.keras.Model):
    ...
    def fit(self, train_data, test_data, test_label, batch_size, num_epochs, cls_epochs, training):
        ...
        if training:
            ...
            for i in range(int(ratio)):
                ...
                for epoch in range(cls_epochs):
                    ...
                    for batch_data, batch_label in combined_dataset:
                        ...
                        # 使用模型进行预测
                        predictions, _, _, _ = self.classifier(batch_data)
                    ...
                    for test_batch_data, test_batch_label in combined_test_dataset:
                        ...
                        # 更新準確率 (TensorFlow 會自動處理 logits -> argmax)
                        accuracy_metric.update_state(test_batch_label, predictions)

                        # 收集預測與真實標籤，用於 F1-score 計算
                        pred_label = np.argmax(predictions, axis=1)
                        all_pred_labels.extend(pred_label)
                        all_true_labels.extend(test_batch_label.numpy())  # 確保是 numpy array
                        ...
                        # 計算 F1-score（二元分類）
                        epoch_f1 = f1_score(all_true_labels, all_pred_labels)
                    # 取得該 epoch 的平均準確率
                    ...
                    if test_epoch_accuracy > best_acc:
                        ...
                        self.classifier.save_weights(
                            f'./CycleGAN_weight/classifier_g/DTN_temp_best_weights_{Parameter.name}.h5')
                ...
                if best_acc > total_best_acc:
                    ...
                    self.classifier.load_weights(f'./CycleGAN_weight/classifier_g/DTN_temp_best_weights_{Parameter.name}.h5')
                    self.classifier.save_weights(
                        './CycleGAN_weight/classifier_g/DTN_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
            ...
            for test_batch_data, test_batch_label in combined_test_dataset:
                ...
                pred_label = np.argmax(predictions, axis=1)
                correct_predictions += np.sum(pred_label == test_batch_label)
                total_predictions += len(test_batch_label)

                # Collect for F1 calculation
                all_true_labels.extend(test_batch_label)
                all_pred_labels.extend(pred_label)

                # AUROC
                predictions_prob = tf.keras.activations.softmax(predictions, axis=-1)
                predictions_prob = predictions_prob[:, 1]  # 只取正類的機率
                auroc = roc_auc_score(test_batch_label, predictions_prob)
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        # 儲存圖片
        plt.savefig(f'DTN_cycleGAN_{Parameter.name}_TSNE.png', dpi=300, bbox_inches='tight')
        np.savez(
            f'DTN_cycleGAN_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
        ...
        with open(f'DTN_cycleGAN_{Parameter.name}_TSNE_data.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    for run in range(num_runs):
        ...
        for var in gan_model.classifier.LayerLogits.trainable_variables:
    ...
    # 將結果寫入文字檔
    with open(f"DTN_cycleGAN_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 DTN_cycleGAN_{Parameter.name}_results.txt")
