...
from ISDA import ISDA_expansion
...
#               emotion_folder_path = os.path.join(data_path, folder_name, "{}_0.npy".format(folder_name))
    ...
    def build(self, input_shape):
        self.LayerD = [
            ...
            layers.Dense(units=units,
                         ...
                         kernel_initializer=create_initializer(h_data_path),
                         bias_initializer=create_initializer(h_data_path),
                         name='d{}_l2'.format(Parameter.name))
        ]
...
class TotalModel(tf.keras.Model):
    def __init__(self, gen_G_optimizer, gen_F_optimizer, disc_X_optimizer, disc_Y_optimizer):
        ...
        self.function_f = ISDA_expansion.function_f(input_dim=Parameter.hidden_dim, embedding_dim=units, name=Parameter.name)
        self.classifier = ISDA_expansion.classifier_g(embedding_dim=units, num_classes=Parameter.num_classes, name=Parameter.name, units=units)
        self.optimizer = tf.keras.optimizers.Adam(initial_learning_rate)
    ...
    def fit(self, train_data, test_data, test_label, batch_size, num_epochs, cls_epochs, training):
        ...
        if training:
            ...
            for i in range(int(ratio)):
                ...
                for epoch in range(cls_epochs):
                    ...
                    for batch_data, batch_label in combined_dataset:
                        ...
                        # 使用模型进行预测
                        predictions = self.classifier(batch_data, training=False)
                    ...
                    if test_epoch_accuracy > best_acc:
                        ...
                        self.classifier.save_weights(
                            f'./CycleGAN_weight/classifier_g/ISDA_temp_best_weights_{Parameter.name}.h5')

                        # print("Trainable variables for classifier.LayerLogits:")
                        # for var in self.classifier.Layers.trainable_variables:
                ...
                if best_acc > total_best_acc:
                    ...
                    self.classifier.load_weights(
                        f'./CycleGAN_weight/classifier_g/ISDA_temp_best_weights_{Parameter.name}.h5')
                    self.classifier.save_weights(
                        './CycleGAN_weight/classifier_g/ISDA_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
                    # self.classifier.save_weights(
                    #     './CycleGAN_weight/classifier_g/ISDA_g_model_weights_{}.h5'.format(Parameter.name))
            ...
            # plt.plot(range(cls_epochs), total_training_acc, label='training acc')
            # plt.xlabel('Epochs')
            # plt.ylabel('acc')
            # plt.title('classifier train_acc')
            # plt.legend()
            # # Plot the discriminator X loss over epochs
            # plt.plot(range(cls_epochs), training_loss, label='training Loss')
            # plt.xlabel('Epochs')
            # plt.ylabel('Loss')
            # plt.title('classifier loss')
            # plt.legend()
            # plt.savefig('cycle_gan_cls_loss.png')
            # # plt.show()
            # plt.clf()  # 清除當前的圖
            # plt.plot(range(cls_epochs), test_epoch_accuracies, label='test_acc')
            # plt.xlabel('Epochs')
            # plt.ylabel('acc')
            # plt.title('test_acc')
            # plt.legend()
            # plt.savefig('cycle_gan_acc.png')
            # # plt.show()
    ...
    def train_classifier(self, batch_data, batch_labels):
        with tf.GradientTape() as tape:
            ...
            loss = self.criterion(batch_labels, predictions)
            # 計算平均損失
    ...
    def save_all_weights(self, path):
        ...
        # 保存 cyclegan1 的模型權重
        self.cyclegan1.generator_G.save_weights(f"{path}/cyclegan1_generator_G_weights_{Parameter.name}.h5")
        self.cyclegan1.generator_F.save_weights(f"{path}/cyclegan1_generator_F_weights_{Parameter.name}.h5")
        self.cyclegan1.discriminator_X.save_weights(f"{path}/cyclegan1_discriminator_X_weights_{Parameter.name}.h5")
        self.cyclegan1.discriminator_Y.save_weights(f"{path}/cyclegan1_discriminator_Y_weights_{Parameter.name}.h5")

        # 保存 cyclegan2 的模型權重
        self.cyclegan2.generator_G.save_weights(f"{path}/cyclegan2_generator_G_weights_{Parameter.name}.h5")
        self.cyclegan2.generator_F.save_weights(f"{path}/cyclegan2_generator_F_weights_{Parameter.name}.h5")
        self.cyclegan2.discriminator_X.save_weights(f"{path}/cyclegan2_discriminator_X_weights_{Parameter.name}.h5")
        self.cyclegan2.discriminator_Y.save_weights(f"{path}/cyclegan2_discriminator_Y_weights_{Parameter.name}.h5")
    ...
    def load_all_weights(self, path):
        ...
        self.cyclegan1.generator_G.load_weights(f"{path}/cyclegan1_generator_G_weights_{Parameter.name}.h5")
        self.cyclegan1.generator_F.load_weights(f"{path}/cyclegan1_generator_F_weights_{Parameter.name}.h5")
        self.cyclegan1.discriminator_X.load_weights(f"{path}/cyclegan1_discriminator_X_weights_{Parameter.name}.h5")
        self.cyclegan1.discriminator_Y.load_weights(f"{path}/cyclegan1_discriminator_Y_weights_{Parameter.name}.h5")
        ...
        self.cyclegan2.generator_G.load_weights(f"{path}/cyclegan2_generator_G_weights_{Parameter.name}.h5")
        self.cyclegan2.generator_F.load_weights(f"{path}/cyclegan2_generator_F_weights_{Parameter.name}.h5")
        self.cyclegan2.discriminator_X.load_weights(f"{path}/cyclegan2_discriminator_X_weights_{Parameter.name}.h5")
        self.cyclegan2.discriminator_Y.load_weights(f"{path}/cyclegan2_discriminator_Y_weights_{Parameter.name}.h5")
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        # 儲存圖片
        plt.savefig(f'ISDA_cycleGAN_{Parameter.name}_TSNE.png', dpi=300, bbox_inches='tight')
        np.savez(
            f'ISDA_cycleGAN_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
        ...
        with open(f'ISDA_cycleGAN_{Parameter.name}_TSNE_data.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    for run in range(num_runs):
        ...
        name = Parameter.name
        gen_G_optimizer = tf.keras.optimizers.Adam(learning_rate=0.0005, beta_1=0.5)
        gen_F_optimizer = tf.keras.optimizers.Adam(learning_rate=0.0005, beta_1=0.5)
        disc_X_optimizer = tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0.5)
        disc_Y_optimizer = tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0.5)
        ...
        classifier.load_weights('./ISDA_weight/classifier_g/g_model_weights_{}.h5'.format(Parameter.name))
        # classifier.load_weights('./CycleGAN_weight/classifier_g/ISDA_g_model_weights_{}.h5'.format(Parameter.name))
    ...
    # 將結果寫入文字檔
    with open(f"ISDA_cycleGAN_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 ISDA_cycleGAN_{Parameter.name}_results.txt")
