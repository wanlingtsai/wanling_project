...
class Jam_GAN(tf.keras.Model):
    def __init__(self, input_dim):
        ...
        self.function_f = Baseline.function_f(input_dim=Parameter.hidden_dim, embedding_dim=units,
                        name=Parameter.name)
        self.classifier = Baseline.classifier_g(input_dim=units, num_classes=Parameter.num_classes,
                        name=Parameter.name)
        ...
        self.discriminator_optimizer = optimizers.Adam(learning_rate=0.0003, beta_1=0.5)
    ...
    def call(self, inputs, training=False):
        ...
        generated_1 = self.generator1(noise1, training=True)
        generated_2 = self.generator2(noise2, training=True)
        ...
        if training:
            ...
            for step in range(ratio):
                ...
                generated_labels = tf.concat(
                    [tf.zeros_like(generated_data_0[:, 0], dtype=tf.int32), tf.ones_like(generated_data_1[:, 0], dtype=tf.int32)], axis=0)
                ...
                #     './ours_weight/classifier_g/Baseline_g_model_weights_{}.h5'.format(Parameter.name))
                ...
                for epoch in range(cls_epoch):
                    ...
                    if (epoch + 1) % 1 == 0:
                        ...
                        if acc > best_acc:
                            ...
                            # 儲存暫時最佳分類器權重
                            self.classifier.save_weights(f'./ours_weight/classifier_g/Baseline_temp_best_weights_{Parameter.name}.h5')

                if best_acc > total_best_acc:
                    total_best_acc, total_best_auroc, total_best_f1 = best_acc, best_auroc, best_f1
                    self.classifier.load_weights(f'./ours_weight/classifier_g/Baseline_temp_best_weights_{Parameter.name}.h5')
                    self.classifier.save_weights(
                        './ours_weight/classifier_g/Baseline_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
            ...
            self.classifier.load_weights('./ours_weight/classifier_g/Baseline_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
    ...
    def evaluate(self, val_data, val_label):
        ...
        # 目前你的 val_labels 應該是一個字典，需要取出其中的 ndarray
        correct_count = np.sum(predictions_new == val_label)
        total_count = len(val_label)
        accuracy = correct_count / total_count
        f1 = f1_score(val_label, predictions_new)
        predictions_prob = tf.keras.activations.softmax(predictions, axis=-1)[:, 1]
        auroc = roc_auc_score(val_label, predictions_prob)
    ...
    def save_all_weights(self, save_dir):
        ...
        for i, generator in enumerate(self.Generator_list):
            ...
            file_path = f"{dir_path}/Baseline_{Parameter.name}_G_weights"
        ...
        for i, discriminator in enumerate(self.Discriminator_list):
            ...
            file_path = f"{dir_path}/Baseline_{Parameter.name}_D_weights"
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)  # 创建目录
            if os.path.exists(file_path):
                os.remove(file_path)  # 删除已经存在的文件，确保不会重复创建组
            ...
            file_path = f"{dir_path}/Baseline_{Parameter.name}_Drf_weights"
            # 確保目錄存在
            if not os.path.exists(dir_path):
        ...
        file_path = f"{dir_path}/Baseline_{Parameter.name}_J_weights"
        # 確保目錄存在
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        # 如果檔案已經存在，先刪除確保不重複
    ...
    def load_all_weights(self, save_dir):
        for i, generator in enumerate(self.Generator_list):
            ...
            file_path = f"{save_dir}/generator/generator_{i}/Baseline_{Parameter.name}_G_weights"
            print(i)
        ...
        for i, discriminator in enumerate(self.Discriminator_list):
            ...
            file_path = f"{save_dir}/discriminator/discriminator_{i}/Baseline_{Parameter.name}_D_weights"
        ...
        # 設定要載入的權重路徑
        file_path = f"{save_dir}/Discriminator_rf/Drf/Baseline_{Parameter.name}_Drf_weights"
        ...
        # 設定要載入的權重路徑
        file_path = f"{save_dir}/jammer/jammer/Baseline_{Parameter.name}_J_weights"
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        plt.savefig(f'baseline_ours_{Parameter.name}_TSNE.png', dpi=300, bbox_inches='tight')
        np.savez(
            f'baseline_ours_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
        ...
        with open(f'baseline_ours_{Parameter.name}_TSNE_data.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    # 將結果寫入文字檔
    with open(f"Baseline_ours_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 Baseline_ours_{Parameter.name}_results.txt")
