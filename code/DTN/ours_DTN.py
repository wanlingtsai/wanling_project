...
class Jam_GAN(tf.keras.Model):
    def __init__(self, input_dim):
        ...
        self.function_f = function_f(input_dim=Parameter.hidden_dim, embedding_dim=units,
                        name=Parameter.name)
        self.classifier = classifier_g(embedding_dim=units, num_classes=Parameter.num_classes,
                        name=Parameter.name)
        ...
        self.discriminator_rf_optimizer = optimizers.Adam(learning_rate=0.0002, beta_1=0.5)
    ...
    def fit(self, train_data, train_label, test_data, test_label, batch_size, num_epochs, cls_epochs, mode, training=True):
        ...
        if training:
            ...
            for step in range(ratio):
                ...
                for epoch in range(gan_epochs):
                    ...
                    if mode == 'gd':
                        ...
                        self.d_ratio, self.g_ratio, self.j_ratio = 5, 5, 5
                ...
                #     './ours_weight/classifier_g/DTN_g_model_weights_{}.h5'.format(Parameter.name))
                ...
                for epoch in range(cls_epoch):
                    ...
                    if (epoch + 1) % 1 == 0:
                        ...
                        for batch_x, batch_y in test_ds:
                            preds, _, _, _ = self.classifier(batch_x, training=False)
                        ...
                        if acc > best_acc:
                            ...
                            # 儲存暫時最佳分類器權重
                            self.classifier.save_weights(f'./ours_weight/classifier_g/DTN_temp_best_weights_{Parameter.name}.h5')

                if best_acc > total_best_acc:
                    total_best_acc, total_best_auroc, total_best_f1 = best_acc, best_auroc, best_f1
                    self.classifier.load_weights(f'./ours_weight/classifier_g/DTN_temp_best_weights_{Parameter.name}.h5')
                    self.classifier.save_weights(
                        './ours_weight/classifier_g/DTN_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
            ...
            self.classifier.load_weights('./ours_weight/classifier_g/DTN_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
    ...
    def evaluate(self, val_data, val_labels):
        predictions, _, _, _ = self.classifier(val_data, training=False)
        # calculate acc
        correct_count = 0
        total_count = 0
    ...
    def save_all_weights(self, save_dir):
        ...
        for i, generator in enumerate(self.Generator_list):
            ...
            file_path = f"{dir_path}/DTN_{Parameter.name}_G_weights"
        ...
        for i, discriminator in enumerate(self.Discriminator_list):
            ...
            file_path = f"{dir_path}/DTN_{Parameter.name}_D_weights"
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)  # 创建目录
            if os.path.exists(file_path):
                os.remove(file_path)  # 删除已经存在的文件，确保不会重复创建组
            ...
            file_path = f"{dir_path}/DTN_{Parameter.name}_Drf_weights"
            # 確保目錄存在
            if not os.path.exists(dir_path):
        ...
        file_path = f"{dir_path}/DTN_{Parameter.name}_J_weights"
        # 確保目錄存在
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        # 如果檔案已經存在，先刪除確保不重複
    ...
    def load_all_weights(self, save_dir):
        for i, generator in enumerate(self.Generator_list):
            ...
            file_path = f"{save_dir}/generator/generator_{i}/DTN_{Parameter.name}_G_weights"
            print(i)
        ...
        for i, discriminator in enumerate(self.Discriminator_list):
            ...
            file_path = f"{save_dir}/discriminator/discriminator_{i}/DTN_{Parameter.name}_D_weights"
        ...
        # 設定要載入的權重路徑
        file_path = f"{save_dir}/Discriminator_rf/Drf/DTN_{Parameter.name}_Drf_weights"
        ...
        # 設定要載入的權重路徑
        file_path = f"{save_dir}/jammer/jammer/DTN_{Parameter.name}_J_weights"
    ...
    def plot_tsne(self, org_data, org_labels, generated_data, generated_labels):
        # 合併原始資料與生成資料
        combined_data = np.concatenate([org_data, generated_data], axis=0)
        combined_labels = np.concatenate([org_labels, generated_labels], axis=0)
        ...
        gen_tsne = tsne_results[len(org_data):]
        plt.figure(figsize=(8, 6))
        # 繪製原始資料點
        plt.scatter(org_tsne[org_labels == 0][:, 0], org_tsne[org_labels == 0][:, 1],
                    color='blue', marker='o', label='Original Class 0')
        plt.scatter(org_tsne[org_labels == 1][:, 0], org_tsne[org_labels == 1][:, 1],
                    color='blue', marker='^', label='Original Class 1')

        # 繪製生成資料點
        plt.scatter(gen_tsne[generated_labels == 0][:, 0], gen_tsne[generated_labels == 0][:, 1],
                    color='red', marker='o', label='Generated Class 0')
        plt.scatter(gen_tsne[generated_labels == 1][:, 0], gen_tsne[generated_labels == 1][:, 1],
                    color='red', marker='^', label='Generated Class 1')

        # 圖片細節設定
        plt.title('T-SNE of Normalized Original and Generated Data')
        plt.legend()
        plt.show()
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        plt.savefig(f'DTN_ours_{Parameter.name}_TSNE.png', dpi=300, bbox_inches='tight')
        np.savez(
            f'DTN_ours_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
...
if __name__ == '__main__':
    ...
    for run in range(num_runs):
        ...
        for layer in f_model.Layers:
            if isinstance(layer, tf.keras.layers.BatchNormalization):
                print("Moving mean:", layer.moving_mean.numpy())
                print("Moving var :", layer.moving_variance.numpy())
        # 印出新模型的摘要
        f_model.summary()
    ...
    # 將結果寫入文字檔
    with open(f"DTN_ours_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 DTN_ours_{Parameter.name}_results.txt")
