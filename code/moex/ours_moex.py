...
class Jam_GAN(tf.keras.Model):
    def __init__(self, input_dim):
        ...
        self.function_f = moex_expansion.function_f(input_dim=Parameter.hidden_dim, embedding_dim=units,
                        name=Parameter.name)
        self.classifier = moex_expansion.classifier_g(input_dim=units, num_classes=Parameter.num_classes,
                        name=Parameter.name)
        ...
        self.acc = tf.keras.metrics.SparseCategoricalAccuracy()
        self.optimizer = optimizers.Adam(initial_learning_rate, beta_1=0.5)
    ...
    def fit(self, train_data, train_label, test_data, test_label, batch_size, num_epochs, cls_epochs, mode, training=True):
        ...
        if training:
            ...
            for step in range(ratio):
                ...
                #     './ours_weight/classifier_g/moex_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
                ...
                for epoch in range(cls_epoch):
                    ...
                    if (epoch + 1) % 1 == 0:
                        ...
                        if acc > best_acc:
                            ...
                            # 儲存暫時最佳分類器權重
                            self.classifier.save_weights('./ours_weight/classifier_g/moex_temp_best_weights.h5')

                if best_acc > total_best_acc:
                    total_best_acc, total_best_auroc, total_best_f1 = best_acc, best_auroc, best_f1
                    self.classifier.load_weights('./ours_weight/classifier_g/moex_temp_best_weights.h5')
    ...
    def save_all_weights(self, save_dir):
        ...
        for i, generator in enumerate(self.Generator_list):
            ...
            file_path = f"{dir_path}/moex_{Parameter.name}_G_weights"
        ...
        for i, discriminator in enumerate(self.Discriminator_list):
            ...
            file_path = f"{dir_path}/moex_{Parameter.name}_D_weights"
            if not os.path.exists(dir_path):
                os.makedirs(dir_path)  # 创建目录
            if os.path.exists(file_path):
                os.remove(file_path)  # 删除已经存在的文件，确保不会重复创建组
            ...
            file_path = f"{dir_path}/moex_{Parameter.name}_Drf_weights"
            # 確保目錄存在
            if not os.path.exists(dir_path):
        ...
        file_path = f"{dir_path}/moex_{Parameter.name}_J_weights"
        # 確保目錄存在
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        # 如果檔案已經存在，先刪除確保不重複
    ...
    def load_all_weights(self, save_dir):
        for i, generator in enumerate(self.Generator_list):
            ...
            file_path = f"{save_dir}/generator/generator_{i}/moex_{Parameter.name}_G_weights"
            print(i)
        ...
        for i, discriminator in enumerate(self.Discriminator_list):
            ...
            file_path = f"{save_dir}/discriminator/discriminator_{i}/moex_{Parameter.name}_D_weights"
        ...
        # 設定要載入的權重路徑
        file_path = f"{save_dir}/Discriminator_rf/Drf/moex_{Parameter.name}_Drf_weights"
        ...
        # 設定要載入的權重路徑
        file_path = f"{save_dir}/jammer/jammer/moex_{Parameter.name}_J_weights"
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        # 儲存圖像
        save_path = f"moex_ours_{Parameter.name}_TSNE.png"  # 你可以改成其他格式，例如 "ours_moex_fig.pdf"
        fig.savefig(save_path, dpi=300, bbox_inches='tight')
        print(f"圖像已儲存至 {save_path}")
        np.savez(
            f'moex_ours_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
        ...
        with open(f'moex_ours_{Parameter.name}_TSNE_data.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    # 將結果寫入文字檔
    with open(f"moex_ours_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 moex_ours_{Parameter.name}_results.txt")
