...
class TotalModel(tf.keras.Model):
    ...
    def fit(self, train_data, test_data, test_label, batch_size, num_epochs, cls_epochs, training):
        ...
        if training:
            ...
            for i in range(int(ratio)):
                ...
                # self.classifier.load_weights('./ISDA_weight/classifier_g/g_model_weights_{}.h5'.format(Parameter.name))
                total_training_acc = []
                for epoch in range(cls_epochs):
                    ...
                    test_epoch_f1 = f1_score(all_labels, all_pred_labels)
                    epoch_auroc_mean = np.mean(epoch_auroc)

                    test_epoch_accuracies.append(test_epoch_accuracy)

                    print(
                        f"Epoch {epoch + 1}/{cls_epochs}, Loss: {Epoch_loss:.4f}, Test Accuracy: {test_epoch_accuracy:.4f}, F1: {test_epoch_f1:.4f}, Test AUROC: {epoch_auroc_mean:.4f}")

                    if test_epoch_accuracy > best_acc:
                        best_acc = test_epoch_accuracy
                        best_epoch = epoch
                        best_auroc = epoch_auroc_mean
                        best_f1 = test_epoch_f1
                        ...
                        self.classifier.save_weights(
                            f'./infoGAN_weight/classifier_g/ISDA_temp_best_weights_{Parameter.name}.h5')
                ...
                if best_acc > total_best_acc:
                    ...
                    self.classifier.load_weights(
                        f'./infoGAN_weight/classifier_g/ISDA_temp_best_weights_{Parameter.name}.h5')
                    self.classifier.save_weights(
                        './infoGAN_weight/classifier_g/ISDA_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))

            print("best acc:", total_best_acc)
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        # 儲存圖片
        plt.savefig(f'ISDA_infoGAN_{Parameter.name}_TSNE.png', dpi=300, bbox_inches='tight')
        np.savez(
            f'ISDA_infoGAN_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
        ...
        with open(f'ISDA_infoGAN_{Parameter.name}_TSNE_data.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    # 將結果寫入文字檔
    with open(f"ISDA_infoGAN_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 ISDA_infoGAN_{Parameter.name}_results.txt")
