...
class acgan(tf.keras.Model):
    def __init__(self, latent_dim, label_dim, name):
        ...
        self.d_name = name
        self.function_f = Baseline.function_f(input_dim=Parameter.hidden_dim, embedding_dim=latent_dim, name=Parameter.name)
    ...
    class get_discriminator(tf.keras.Model):
        def __init__(self, latent_dim, name):
            ...
            self.classifier_g = Baseline.classifier_g(latent_dim, num_classes=Parameter.num_classes, name=name, units=units)
            self.d_name = name
            self.latent_dim = latent_dim    #1920
    ...
    def ganfit(self, data_d, labels, val_data, val_labels, batch_size, num_epochs, training):
        ...
        batch_acc_list, epoch_acc_list, all_acc = [], [], []
        ...
        if training:
            ...
            for ep in range(num_epochs):
                ...
                if val_ep_acc > val_best_acc:
                    ...
                    self.d.save_weights('./ACGAN_weight/Baseline_d_weight/Baseline_val_d_weights_{}.h5'.format(Parameter.name))
                    self.g.save_weights('./ACGAN_weight/Baseline_g_weight/Baseline_val_g_weights_{}.h5'.format(Parameter.name))
    ...
    def evaluate(self, val_data, val_label):
        ...
        self.d.load_weights('./ACGAN_weight/Baseline_d_weight/Baseline_val_d_weights_{}.h5'.format(Parameter.name))
        # 模型預測
        ...
        with open(f"baseline_ACGAN_prediction_{Parameter.name}.json", "w") as f:
    ...
    def save_all_weights(self, path):
        """儲存生成器與判別器的權重"""
        # 儲存 Generator 權重
        self.g.save_weights(f"{path}/Baseline_generator_weights.h5")
        # 儲存 Discriminator 權重
        self.d.save_weights(f"{path}/Baseline_discriminator_weights.h5")
        print("生成器與判別器的權重已儲存。")
    ...
    def load_all_weights(self, path):
        ...
        # 載入權重
        self.g.load_weights(f"{path}/Baseline_generator_weights.h5")
        self.d.load_weights(f"{path}/Baseline_discriminator_weights.h5")
        print("生成器與判別器的權重已載入。")
...
if __name__ == '__main__':
    ...
    print("F1 std:", f1_std)
    # 將結果寫入文字檔
    with open(f"baseline_ACGAN_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 baseline_ACGAN_{Parameter.name}_results.txt")
    print("-------------------------------------------------------------")
