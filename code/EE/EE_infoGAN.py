...
class TotalModel(tf.keras.Model):
    ...
    def fit(self, train_data, test_data, test_label, batch_size, num_epochs, cls_epochs, training):
        ...
        if training:
            ...
            for i in range(int(ratio)):
                ...
                # self.classifier.load_weights('./EE_weight/classifier_g/g_model_weights_{}.h5'.format(Parameter.name))
                total_training_acc = []
                for epoch in range(cls_epochs):
                    epoch_loss = []
                    epoch_auroc = 0.0
                    all_preds = []  # 用於 F1-score 計算
                    all_trues = []
                    ...
                    for test_batch_data, test_batch_label in combined_test_dataset:
                        ...
                        all_preds.extend(pred_labels)  # <<< 新增
                        all_trues.extend(test_batch_label.numpy())  # <<< 新增

                        correct_predictions += np.sum(pred_labels == test_batch_label.numpy())
                        ...
                        auroc = roc_auc_score(test_batch_label.numpy(), predictions_prob)
                    ...
                    epoch_f1 = f1_score(all_trues, all_preds, average='macro')  # <<< 新增

                    print(
                        f"Epoch {epoch + 1}/{cls_epochs}, Loss: {Epoch_loss:.4f}, Test Accuracy: {test_epoch_accuracy:.4f}, "
                        f"Test AUROC: {epoch_auroc:.4f}, Test F1: {epoch_f1:.4f}")  # <<< F1 加入輸出

                    if test_epoch_accuracy > best_acc:
                        best_acc = test_epoch_accuracy
                        best_epoch = epoch
                        best_auroc = epoch_auroc
                        best_f1 = epoch_f1  # <<< 新增：最佳 F1-score
                        ...
                        self.classifier.save_weights(f'./infoGAN_weight/classifier_g/EE_temp_best_weights_{Parameter.name}.h5')
                ...
                if best_acc > total_best_acc:
                    ...
                    self.classifier.load_weights(f'./infoGAN_weight/classifier_g/EE_temp_best_weights_{Parameter.name}.h5')
                    self.classifier.save_weights(
                        './infoGAN_weight/classifier_g/EE_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))

                print("best acc = ", best_acc)
                print("best epoch = ", best_epoch + 1)
                print("best f1 = ", best_f1)  # <<< 新增
            ...
            for test_batch_data, test_batch_label in combined_test_dataset:
                ...
                correct_predictions += np.sum(predicted_labels == test_batch_label.numpy())
                total_predictions += len(test_batch_label)

                predictions_prob = tf.keras.activations.softmax(predictions, axis=-1)
                predictions_prob = predictions_prob[:, 1]  # 對應 positive class
                auroc = roc_auc_score(test_batch_label.numpy(), predictions_prob)
            ...
            # 計算 F1-score（你也可以改成 binary 或 weighted）
    ...
    def save_all_weights(self, path):
        ...
        self.infogan1.generator.save_weights(f"{path}/EE_infogan1_generator_weights_{Parameter.name}.h5")
        self.infogan1.discriminator.save_weights(f"{path}/EE_infogan1_discriminator_weights_{Parameter.name}.h5")
        self.infogan1.q_network.save_weights(f"{path}/EE_infogan1_q_network_weights_{Parameter.name}.h5")

        self.infogan2.generator.save_weights(f"{path}/EE_infogan2_generator_weights_{Parameter.name}.h5")
        self.infogan2.discriminator.save_weights(f"{path}/EE_infogan2_discriminator_weights_{Parameter.name}.h5")
        self.infogan2.q_network.save_weights(f"{path}/EE_infogan2_q_network_weights_{Parameter.name}.h5")
        print("所有模型的權重已儲存。")
    ...
    def load_all_weights(self, path):
        ...
        self.infogan1.generator.load_weights(f"{path}/EE_infogan1_generator_weights_{Parameter.name}.h5")
        self.infogan1.discriminator.load_weights(f"{path}/EE_infogan1_discriminator_weights_{Parameter.name}.h5")
        self.infogan1.q_network.load_weights(f"{path}/EE_infogan1_q_network_weights_{Parameter.name}.h5")
        ...
        self.infogan2.generator.load_weights(f"{path}/EE_infogan2_generator_weights_{Parameter.name}.h5")
        self.infogan2.discriminator.load_weights(f"{path}/EE_infogan2_discriminator_weights_{Parameter.name}.h5")
        self.infogan2.q_network.load_weights(f"{path}/EE_infogan2_q_network_weights_{Parameter.name}.h5")
        print("所有模型的權重已載入。")
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        # 儲存圖片
        plt.savefig(f'EE_infoGAN_{Parameter.name}_TSNE.png', dpi=300, bbox_inches='tight')
        np.savez(
            f'EE_infoGAN_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
        ...
        with open(f'EE_infoGAN_{Parameter.name}_TSNE_data.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    for run in range(num_runs):
        ...
        result = gan_model.fit(train_data, test_phi_data_tensor, val_label_tensor,
                                      batch_size=batch_size, num_epochs=gan_epochs,
                                        cls_epochs=cls_epoch, training=False)
    ...
    # 將結果寫入文字檔
    with open(f"EE_infoGAN_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 EE_infoGAN_{Parameter.name}_results.txt")
