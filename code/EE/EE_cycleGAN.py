...
class TotalModel(tf.keras.Model):
    def __init__(self, gen_G_optimizer, gen_F_optimizer, disc_X_optimizer, disc_Y_optimizer):
        ...
        self.function_f = embedding_expansion.function_f(input_dim=Parameter.hidden_dim, embedding_dim=units, name=Parameter.name)
        self.classifier = embedding_expansion.classifier_g(input_dim=units, num_classes=Parameter.num_classes, name=Parameter.name, units=units)
    ...
    def fit(self, train_data, test_data, test_label, batch_size, num_epochs, cls_epochs, training):
        ...
        if training:
            ...
            for i in range(int(ratio)):
                ...
                combined_dataset = combined_dataset.shuffle(buffer_size=3000).batch(batch_size)
                ...
                for epoch in range(cls_epochs):
                    ...
                    for batch_data, batch_label in combined_dataset:
                        ...
                        correct_predictions += np.sum(predicted_labels.numpy() == batch_label.numpy())
                    ...
                    for test_batch_data, test_batch_label in combined_test_dataset:
                        ...
                        correct_predictions += np.sum(pred_labels == test_batch_label)
                    ...
                    print(f"Epoch {epoch + 1}/{cls_epochs}, Loss: {Epoch_loss:.4f}, "
                          f"Test Accuracy: {test_epoch_accuracy:.4f}, Test AUROC: {epoch_auroc:.4f}, F1-score: {test_f1:.4f}")
                    ...
                    if test_epoch_accuracy > best_acc:
                        ...
                        self.classifier.save_weights(f'./CycleGAN_weight/classifier_g/EE_temp_best_weights_{Parameter.name}.h5')
                ...
                if best_acc > total_best_acc:
                    ...
                    self.classifier.load_weights(f'./CycleGAN_weight/classifier_g/EE_temp_best_weights_{Parameter.name}.h5')
                    self.classifier.save_weights(
                        './CycleGAN_weight/classifier_g/EE_FinalBest_g_model_weights_{}.h5'.format(Parameter.name))
                print("best epoch = ", best_epoch)
                print("best accuracy = ", best_acc)

            print("Total best accuracy = ", total_best_acc)
            print("Total best auroc = ", total_best_auroc)
            print("Total best f1 = ", total_best_f1)
            ...
            eval_f1 = f1_score(all_true_labels, all_pred_labels)

            print(f"Final Accuracy: {eval_acc:.4f}, AUROC: {eval_auroc:.4f}, F1-score: {eval_f1:.4f}")
            return {
                "acc": eval_acc,
                "auroc": eval_auroc,
                "f1": eval_f1
    ...
    def plot_tsne_3d(self, org_data, org_labels, generated_data, generated_labels, test_data, test_labels):
        ...
        # 儲存圖片
        plt.savefig(f'EE_cycleGAN_{Parameter.name}_TSNE.png', dpi=300, bbox_inches='tight')
        np.savez(
            f'EE_cycleGAN_{Parameter.name}_TSNE_data.npz',
            org_tsne=org_tsne,
            gen_tsne=gen_tsne,
            test_tsne=test_tsne,
        ...
        with open(f'EE_cycleGAN_{Parameter.name}_TSNE_data.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    all_runs_acc, all_runs_auroc, all_runs_f1 = 0.0, 0.0, 0.0
    acc_std, auroc_std, f1_std = 0.0, 0.0, 0.0
    gan_epochs = 50
    cls_epoch = 100
    batch_size = 64
    total_gan_epoch = 200
    ...
    for run in range(num_runs):
        ...
        print("Trainable variables for classifier_g.Layerss:")
    ...
    # 將結果寫入文字檔
    with open(f"EE_cycleGAN_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 EE_cycleGAN_{Parameter.name}_results.txt")
