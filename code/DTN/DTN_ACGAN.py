...
class acgan(tf.keras.Model):
    def __init__(self, latent_dim, label_dim, name):
        ...
        self.d_name = name
        self.function_f = function_f(input_dim=Parameter.hidden_dim, embedding_dim=latent_dim, name=Parameter.name)
    ...
    class get_discriminator(tf.keras.Model):
        def __init__(self, latent_dim, name):
            ...
            self.classifier_g = classifier_g(latent_dim, num_classes=Parameter.num_classes, name=name, units=units)
            self.Sigma = self.classifier_g.LayerSigma
            self.LayerLogits = self.classifier_g.LayerLogits
            self.d_name = name
            self.latent_dim = latent_dim    #1920
        ...
        def call(self, inputs, training=True):
            ...
            x_2 = inputs
            o_class, _, _, _ = self.classifier_g.call(x_2, training=training)
    ...
    def train_g(self, random_img_label):    #random_img_label:(32,)
        ...
        grads = tape.gradient(loss, self.g.LayerG.trainable_variables)
        self.opt_g.apply_gradients(zip(grads, self.g.LayerG.trainable_variables))
        ...
        if training:
            ...
            for ep in range(num_epochs):
                ...
                for val_batch_data, val_batch_labels in val_ds:
                    ...
                    # 計算 F1 分數（二分類：需要指定 average='binary'）
                    batch_f1 = f1_score(val_labels_array, predictions_new, average='binary')
                ...
                if val_ep_acc > val_best_acc:
                    ...
                    val_best_f1 = val_ep_f1  # 儲存最佳 F1
                    val_best_epoch = ep + 1
                    print("g:", g_bool_acc, "| d:", d_bool_acc)
                    best_acc_g = g_bool_acc
                    best_acc_d = d_bool_acc
                    self.d.save_weights('./ACGAN_weight/DTN_d_weight/DTN_val_d_weights_{}.h5'.format(Parameter.name))
            ...
            self.d.load_weights('./ACGAN_weight/DTN_d_weight/DTN_val_d_weights_{}.h5'.format(Parameter.name))
    ...
    def save_all_weights(self, path):
        """儲存生成器與判別器的權重"""
        # 儲存 Generator 權重
        self.g.save_weights(f"{path}/DTN_generator_weights_{Parameter.name}.h5")
        # 儲存 Discriminator 權重
        self.d.save_weights(f"{path}/DTN_discriminator_weights_{Parameter.name}.h5")
        print("生成器與判別器的權重已儲存。")
    ...
    def load_all_weights(self, path):
        ...
        # 載入權重
        self.g.load_weights(f"{path}/DTN_generator_weights_{Parameter.name}.h5")
        self.d.load_weights(f"{path}/DTN_discriminator_weights_{Parameter.name}.h5")
        print("生成器與判別器的權重已載入。")
...
if __name__ == '__main__':
    ...
    for run in range(num_runs):
        ...
        d_model.load_weights('./DTN_weight/classifier_g/g_model_weights_{}.h5'.format(Parameter.name))
    ...
    print("auroc std:", std_au)
    print("Mean f1:", mean_f1)
    print("f1_std:", f1_std)
    # 將結果寫入文字檔
    with open(f"DTN_ACGAN_{Parameter.name}_results.txt", "w") as file:
    ...
    print(f"結果已寫入 DTN_ACGAN_{Parameter.name}_results.txt")
    print("-------------------------------------------------------------")
